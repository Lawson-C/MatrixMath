...
typename std::enable_if<(Spalten != Zeilen) || (Spalten < 2) || (Spalten > 4), Vector<Spalten>>::type
inline operator*(Matrix<Zeilen, Spalten> &m, Vector<Spalten> &v)
{
...
inline Vector<2> operator*(Matrix<2, 2> &m, Vector<2> &v)
{
    return Vector2(m.get(0, 0) * v.get(0) + m.get(0, 1) * v.get(1),
                   m.get(1, 0) * v.get(0) + m.get(1, 1) * v.get(1));
};

inline Vector3 operator*(Matrix33 &m, Vector3 &v)
{
...
typename std::enable_if<(Länge < 2) || (Länge > 4), Vector<Länge>>::type
inline operator*(Vector<Länge> &v, Matrix<Länge, Spalten> &m)
{
    Vector<Spalten> result = Vector<Spalten>();
...
template <int Zeilen>
typename std::enable_if<Zeilen != 2, Vector<Zeilen>>::type
inline operator*(Vector2 &v, Matrix<Zeilen, 2> &m)
{
...
inline Vector2 &operator*=(Vector2 &v, Matrix22 &m)
{
...
template <int Zeilen>
typename std::enable_if<Zeilen != 3, Vector<Zeilen>>::type
inline operator*(Vector3 &v, Matrix<Zeilen, 3> &m)
{
    Vector<Zeilen> result = Vector<Zeilen>();
    for (int i = 0; i < Zeilen; i++)
    {
...
inline Vector3 &operator*=(Vector3 &v, Matrix33 &m)
{
...
template <int Zeilen>
typename std::enable_if<Zeilen != 4, Vector<Zeilen>>::type
inline operator*(Vector4 &v, Matrix<Zeilen, 4> &m)
{
    Vector<Zeilen> result = Vector<Zeilen>();
    for (int i = 0; i < Zeilen; i++)
    {
...
inline Vector4 &operator*=(Vector4 &v, Matrix44 &m)
{
